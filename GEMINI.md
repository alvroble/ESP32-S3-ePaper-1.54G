...
Este documento describe las reglas críticas de hardware y arquitectura para cualquier proyecto que utilice la placa **Waveshare ESP32-S3 ePaper 1.54G** (y placas compatibles de la familia Waveshare ePaper).
...
- **GPIO 17 (`VBAT_PWR_PIN`) - Retención de Batería (Power Latch)**:
  ...
  - **REGLA**: El firmware **DEBE poner inmediatamente `GPIO 17 = 1` (`HIGH`)** en `user_app_init()` / `app_main()` para enclavar la alimentación.
