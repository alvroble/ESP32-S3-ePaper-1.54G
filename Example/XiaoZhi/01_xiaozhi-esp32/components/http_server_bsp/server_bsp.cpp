...
esp_err_t get_html_callback(httpd_req_t *req) {
    ...
    while (1) {
        str_len = s_example_read_from_offset(Name_str,resp_str,SEND_LEN_MAX,str_respLen);
        if (str_len) {
...
esp_err_t post_dataup_callback(httpd_req_t *req) {
    ...
    s_example_wriet_from_offset("/sdcard/02_sys_ap_img/user_send.bmp", NULL, 0, 0);
    while (remaining > 0) {
        ...
        size_t req_len = s_example_wriet_from_offset("/sdcard/02_sys_ap_img/user_send.bmp", buf, ret, 1);
...
static void wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        xEventGroupSetBits(server_groups, set_bit_button(4));//重置睡眠倒计时
    } else if (event_id == WIFI_EVENT_AP_STADISCONNECTED) {
        xEventGroupSetBits(server_groups, set_bit_button(5));//进入深睡眠
