...
static void key1_button_user_Task(void *arg) {
    esp_err_t ret;
    uint8_t   Mode = 0;
    xEventGroupWaitBits(boot_groups, set_bit_button(3), pdTRUE, pdFALSE, portMAX_DELAY);
    ...
    for (;;) {
        ...
        if (get_bit_button(even, 1)) {
            if (Mode > 0) {
                ...
                dev_audio->Set_CodecReg("es7210", 0x00, 0x32);
                even = xEventGroupWaitBits(boot_groups, set_bit_button(3), pdTRUE, pdFALSE, portMAX_DELAY);
