    ...
    for (;;)
    {
        ...
        if (get_bit_button(even, 0))
        {
            ...
            if (pdTRUE == xSemaphoreTake(epaper_gui_semapHandle, 2000))
            {
                ...
                if (g_sdcard_ok) 
                {
                    // SD card OK → read image from SD card
                ...
                {
                    // No SD card → show built-in image
                    ESP_LOGI("SYSTEM", "SD card not found, display default image");
...
void User_Basic_mode_app_init(void)
{
    ...
    if (g_sdcard_ok && (13 * 60) == basic_rtc_set_time)
    {
