...
#define ext_wakeup_pin_1      GPIO_NUM_0    // Boot键
#define ext_wakeup_pin_pwr    GPIO_NUM_18   // PWR键
...
static void Network_user_Task(void *arg) {
    ...
    for (;;) {
        ...
        } else if (get_bit_button(even, 5)) 
        {
            ...
            const uint64_t ext_wakeup_pin_mask = (1ULL << ext_wakeup_pin_1) | (1ULL << ext_wakeup_pin_pwr);
            ESP_ERROR_CHECK(esp_sleep_enable_ext1_wakeup_io(
                ext_wakeup_pin_mask, ESP_EXT1_WAKEUP_ANY_LOW)); 

            ESP_ERROR_CHECK(rtc_gpio_pulldown_dis(ext_wakeup_pin_1));
            ESP_ERROR_CHECK(rtc_gpio_pullup_en(ext_wakeup_pin_1));

            ESP_ERROR_CHECK(rtc_gpio_pulldown_dis(ext_wakeup_pin_pwr));
            ESP_ERROR_CHECK(rtc_gpio_pullup_en(ext_wakeup_pin_pwr));
...
static void get_wakeup_gpio(void) {
    ...
    if (wakeup_reason == ESP_SLEEP_WAKEUP_TIMER) {
        ...
        if (wake_pin & (1ULL << ext_wakeup_pin_1)) {
            ESP_LOGI(TAG, "Wake source: Boot key wake");
        } else if (wake_pin & (1ULL << ext_wakeup_pin_pwr)) {
            ESP_LOGI(TAG, "Wake source: PWR key wake (GPIO18)");
        }
    } else {
        xEventGroupSetBits(Green_led_Mode_queue, set_bit_button(6));
        Green_led_arg = 1;
        ...
        if(!g_sdcard_ok)        
        {
            ...
            Paint_DrawString_CN(10, 10, "未检测到SD卡", &Font14CN, EPD_1IN54G_RED, EPD_1IN54G_WHITE);
            Paint_DrawString_CN(10, 40, "进入休眠模式", &Font14CN, EPD_1IN54G_RED, EPD_1IN54G_WHITE);
            Paint_DrawString_EN(10, 70, "No SD card detected.", &Font24,EPD_1IN54G_WHITE,EPD_1IN54G_RED);
            Paint_DrawString_EN(10, 120, "Enter sleepmode.", &Font24,EPD_1IN54G_WHITE,EPD_1IN54G_RED);
            epaper_port_display(epd_blackImage);
            ...
            Paint_DrawString_EN(10, 10, "wifi SSID:", &Font24,EPD_1IN54G_WHITE,EPD_1IN54G_BLACK);
            Paint_DrawString_EN(10, 30, "esp_network", &Font24,EPD_1IN54G_WHITE,EPD_1IN54G_YELLOW);
            Paint_DrawString_EN(10, 50, "password:", &Font24,EPD_1IN54G_WHITE,EPD_1IN54G_BLACK);
            Paint_DrawString_EN(10, 70, "1234567890", &Font24,EPD_1IN54G_WHITE,EPD_1IN54G_YELLOW);
            epaper_port_display(epd_blackImage);
            vTaskDelay(pdMS_TO_TICKS(2000));
        }
    }
    ...
    if (ESP_SLEEP_WAKEUP_EXT1 == wakeup_reason) {
        ...
        if (wakeup_pins == 0) return;

        if ((wakeup_pins & (1ULL << ext_wakeup_pin_1)) || (wakeup_pins & (1ULL << ext_wakeup_pin_pwr))) {
...
void User_Network_mode_app_init(void) {
    ...
    xTaskCreate(boot_button_user_Task, "boot_button_user_Task", 5 * 1024, NULL, 2, NULL); 
    get_wakeup_gpio(); 
}